...
export default async function ChatPage({
  ...
  const [threadRes, meRes] = await Promise.all([
    fetch(`${BASE}/api/messages/threads/${id}`, { headers, cache: "no-store" }),
    ...
    <main className="bg-[#F2F2EF] min-h-screen">
      <div className="max-w-[680px] mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-3 mb-5">
          <Link
            href="/inbox"
            className="w-9 h-9 flex items-center justify-center rounded-full bg-white border border-[#E2E2DC] hover:bg-[#F2F2EF] transition-colors shrink-0"
        ...
        <div className="bg-white rounded-[14px] border border-[#E2E2DC] p-4 shadow-[0_1px_3px_rgba(26,26,24,0.06)]">
          <ChatClient
            threadId={thread.id}
